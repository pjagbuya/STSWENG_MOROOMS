...
function getRoomSchedules(room, checkDay, roomSchedules_data) {
    ...
    for (let roomSchedule_data of roomSchedules_data) {
        if (
            roomSchedule_data.room == room &&
            roomSchedule_data.day == checkDay
        ) {
...
function checkAvailability(schedules, reservations, roomSchedules, currentHour, isCurrentDate, currentFullDate, currentDay) {
    ...
    for (let i = 0; i < reservations.length; i++) {
        ...
        for(let j = 0; j < rooms.length; j++){
            if(reservations[i].room.roomSet == rooms[j][0]){
                isExist = true
                rooms[j].push({room: reservations[i].room, reservations: reservations[i].reservations})
            }
        }
        if(!isExist){
            rooms.push([])
            rooms[rooms.length-1].push(reservations[i].room.roomSet)
            rooms[rooms.length-1].push({room: reservations[i].room, reservations: reservations[i].reservations, roomSchedules: []})
        }
    }
    ...
    for(let i = 0; i < rooms.length; i++){
        ...
        for (let j = 1; j < rooms[i].length; j++){
            for(let k = 0; k < rooms[i][j].reservations.length; k++){
                for (let l = rooms[i][j].reservations[k].startTime; l < rooms[i][j].reservations[k].endTime; l++) {
                    time[i][j-1][l] = 1;
                }
            }
        }
        ...
        for (let j = 1; j < rooms[i].length; j++){
            for(let k = 0; k < rooms[i][j].roomSchedules.length; k++){
                for (let l = rooms[i][j].roomSchedules[k].startTime; l < rooms[i][j].roomSchedules[k].endTime; l++) {
                    time[i][j-1][l] = 1;
                }
            }
        }
...
function recommenderSystems(
    ...
    for (let i = current.getDay(); i < 7; i++) {
        ...
        let checkFullDate = current.getFullYear().toString().concat("-").concat((current.getMonth()+1).toLocaleString('en-US', {minimumIntegerDigits: 2, useGrouping:false})).concat("-").concat(checkDate.toLocaleString('en-US', {minimumIntegerDigits: 2, useGrouping:false}));
