...
export async function addRoomAction(prevState, formData) {
  ...
  try {
    ...
    const logData = {
      roomID,
      name: rpcPayload.p_room_name,
      details: rpcPayload.p_room_details,
    };
    ...
    await APILogger.log(action, method, table, userId, payload, {
      message: 'Unexpected top-level error in addRoomAction',
      error,
    });
    return {
      status: 'error',
...
export async function deleteRoomAction(id) {
  ...
  try {
    ...
    const roomData = await callRoomRpc(
      'get_room_by_id',
      payload,
      action,
      userId,
    ...
    // LOG: Unexpected top-level error
    await APILogger.log(action, method, table, userId, payload, {
      message: 'Unexpected top-level error in deleteRoomAction',
      err,
    });
  ...
  try {
    ...
    if (!imageFile || imageFile.size === 0) {
      ...
      await APILogger.log(
        ...
        table,
        userId,
        { id, image_status: 'not_changed/cleared' },
        null,
      );
      revalidatePath('/rooms');
      return { status: 'success' };
    ...
    if (imageFile.name === id) {
      ...
      await APILogger.log(
        ...
        table,
        userId,
        { id, image_status: 'file_name_matched' },
        null,
      );
      revalidatePath('/rooms');
      return { status: 'success' };
    ...
    // LOG: Unexpected top-level error
    await APILogger.log(action, method, table, userId, rpcPayload, {
      message: 'Unexpected top-level error in editRoomAction',
      error,
    });
    return { status: 'error', message: 'An unexpected server error occurred.' };
  }
